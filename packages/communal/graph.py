...
async def engage(graph, node_id, engagement_handle):
    response = await graph.execute_query(
        """
        MATCH (engagee)
...
async def query_nutrient_without_seeking_spore(graph):
    response = await graph.execute_query(
        """
        MATCH (nutrient:Nutrient)
...
async def query_nutrient_topic_and_context_from_spore(graph, spore_id):
    response = await graph.execute_query(
        """
...
async def create_initial_region(graph, spore_id, inquiry):
    await graph.execute_query(
        """
        MATCH (spore:Spore)
        WHERE elementId(spore) = $spore_id
...
async def create_region(graph, source_region_id, source_claim_id, inquiry):
    await graph.execute_query(
        """
...
async def query_inquiry_without_finding(graph):
    response = await graph.execute_query(
        """
...
async def create_finding(graph, region_id, reasoning, content, citations):
    await graph.execute_query(
        """
        MATCH (region:Region)
...
async def query_finding_without_claims(graph):
    response = await graph.execute_query(
        """
...
async def create_claim(graph, region_id, content, citations):
    await graph.execute_query(
        """
        MATCH (region:Region)
        WHERE elementId(region) = $region_id
...
async def query_claim_without_relevance_or_terminus(graph):
    response = await graph.execute_query(
        """
        MATCH (claim:Claim)
...
async def bind_claim_to_nutrient(graph, claim_id, nutrient_id):
    await graph.execute_query(
        """
...
async def create_terminus(graph, claim_id):
    await graph.execute_query(
        """
        MATCH (claim:Claim)
        WHERE elementId(claim) = $claim_id
...
async def query_relevant_claim_without_region(graph):
    response = await graph.execute_query(
        """
...
async def query_nutrient_topic_and_context_from_claim(graph, claim_id):
    response = await graph.execute_query(
        """
